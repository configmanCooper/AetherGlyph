...
**Version 1.13.3 — feature complete.** The castle-styled title page now stages an
...
notice when a newer Store version is available. Version 1.10.0 hardens every
...
available for ready-up rematches. Version 1.13.0 added persistent Glyph world
rankings, temporary username/PIN accounts, separate ranked and unranked queues,
five-second opt-in Easy/Medium/Hard AI fallback opponents, four wizard emojis,
and read-only ranked-match spectating. It also turns every private room into a
...
`com.configmancooper.aetherglyph`, API 24 → 36, versionCode 11303 /
versionName 1.13.3). Both the native package and installable web app offer
...
- **Statuses & buffs for real play.** Soaked, Static, Sundered, Weakened, Marked,
  ...
  are all simulated.
- **Control lockouts.** Frozen and Stunned stop Aether/Stamina regeneration and
  prevent casting, Focus, Brace, movement, and Dodge. Knocked Down prevents
  movement, Focus, Brace, and Dodge while still allowing spell casts.
...
- **Capacitor Android project (checked in).** `com.configmancooper.aetherglyph`,
  ...
  `versionCode 11303` / `versionName 1.13.3`, HTTPS production traffic plus
...
- **Assets + store.** `npm run android:assets` generates the app icon, adaptive
  ...
  `npm run android:screenshots` renders complete full/demo phone, tablet, and PC screenshot sets. Listing,
...
- Paid game: $4.99 one-time purchase
